...
class damageCalculator extends AppWindow {
  ...
  public static setTableRow(data: temListInterface, location: number, fields: string[]) {
    ...
    if (location === 1) {
      ...
      if (data.isAvail === temDataAvailiable.no) {
        fields.forEach(element => {
          damageCalculator.table1[element].innerHTML = '';
        });
      } else if (data.isAvail === temDataAvailiable.yes) {
        fields.forEach(element => {
      ...
      if (data.isAvail === temDataAvailiable.no) {
        fields.forEach(element => {
          damageCalculator.table2[element].innerHTML = '';
        });
        return;
      } else {
        fields.forEach(element => {
